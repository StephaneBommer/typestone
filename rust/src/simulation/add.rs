...
#[wasm_bindgen]
impl Simulation {
    pub fn add_wire(&mut self, positions: Array) -> u32 {
    ...
    pub fn add_and_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
        let and_gate = AndGate {
            gate: TwoInputsGate::new(self.composants.len(), position),
        };
        self.composants.push(ComposantsEnum::AndGate(and_gate));
        self.composants.len() as u32 - 1
    }

    pub fn add_or_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
        let or_gate = OrGate {
            gate: TwoInputsGate::new(self.composants.len(), position),
        };
    ...
    pub fn add_xor_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
    ...
    pub fn add_not_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
        let not_gate = NotGate {
            gate: OneInputGate::new(self.composants.len(), position),
        };
        self.composants.push(ComposantsEnum::NotGate(not_gate));
        self.composants.len() as u32 - 1
    }

    pub fn add_buffer_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
    ...
    pub fn add_latch_gate(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
    ...
    pub fn add_timer(&mut self, position: Vec<u32>, ticks: u32) -> u32 {
        let position = [position[0], position[1]];
        let timer_gate = TimerGate::new(OneInputGate::new(self.composants.len(), position), ticks);
        self.composants.push(ComposantsEnum::TimerGate(timer_gate));
        self.composants.len() as u32 - 1
    }

    pub fn add_switch(&mut self, position: Vec<u32>) -> u32 {
        let position = [position[0], position[1]];
